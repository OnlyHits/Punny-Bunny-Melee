...
namespace GMTK
{
    public class Player : BaseBehaviour
    {
        ...
        [SerializeField] private AttackInterface m_attackManager;
        ...
        public AttackInterface GetAttackManager()
        {
            if (m_attackManager == null)
            {
                Debug.LogWarning($"none attack interface set on player {gameObject.name}");
                return null;
            }
